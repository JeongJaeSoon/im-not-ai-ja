...
> [!IMPORTANT]
> このプロジェクトは [epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai) に着想を得た、日本語向けの独立プロジェクトとして始まりました。現在の日本語分類体系、レジスター設計、ポータブルAgent Skillの実装、評価基盤、インストール構成は、このリポジトリで独自に開発しています。GitHubのfork機能は使っていません。初期設計から受けた影響とMITライセンスに基づく帰属表示は [NOTICE.md](NOTICE.md) に記録しています。
...
MIT License。プロジェクトの出発点と初期設計への影響を明示するため、原プロジェクトの著作権表示を保持しています。現在の実装との関係は [NOTICE.md](NOTICE.md) を参照してください。
