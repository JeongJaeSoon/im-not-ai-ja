...
4. 数値、固有名詞、引用、モダリティ、因果、レジスターを固定する。人名・製品名は明示的な保護語として比較する。
5. taxonomy IDが付く箇所だけ、S1→S2の順に最小編集する。
6. `scripts/metrics.py` は5文以上の補助診断にだけ使う。
7. 自己検証6項目を通す。変更率の閾値は100字以上かつ5文以上にだけ適用し、短文では参考値にする。
...
出力は、状態一行、推敲本文、実在するfinding（0〜6件）、保持した表現の理由、不確実箇所の順にする。無修正の0%も成功として扱う。原文にない体験・出典・具体例を足さない。AI検出器回避を保証しない。
